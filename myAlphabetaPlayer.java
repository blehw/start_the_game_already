...
public class myAlphabetaPlayer extends SampleGamer {

	private int maxScore(Role role, MachineState state, int alpha, int beta) throws GoalDefinitionException, TransitionDefinitionException, MoveDefinitionException {
	...
	private int minScore(Role role, Move move, MachineState state, int alpha, int beta) throws GoalDefinitionException, TransitionDefinitionException, MoveDefinitionException {
	...
	private Move bestMove(Role role, MachineState state) throws GoalDefinitionException, TransitionDefinitionException, MoveDefinitionException {
		...
		int score = 0;
		for (int i = 0; i < moves.size(); i++) {
			int result = minScore(role, moves.get(i), state, 0, 100);
			if (result > score) {
				score = result;
				action = moves.get(i);
			}
